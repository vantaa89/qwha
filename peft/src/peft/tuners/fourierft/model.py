    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        **optional_kwargs,
        ...
        channel_wise = fourierft_config.channel_wise
        cosine_transform = fourierft_config.cosine_transform
        hadamard_transform = fourierft_config.hadamard_transform
    ...
    def state_dict(self, *args, **kwargs):
        """
        Overloads `state_dict()` method for the model's `state_dict` to incorporate `fourierft_indices`.
        Since our proposed FourierFT intialization requires the spectrum indices to vary by layer, it is necessary to store the indices in the state_dict
        """

        state_dict = super().state_dict(*args, **kwargs)
        for name, module in self.named_modules():
            if isinstance(module, FourierFTLayer):
                for adapter_name in module.fourierft_indices.keys():
                    state_dict_key = f"base_model.{name}.fourierft_indices"
                    state_dict[state_dict_key] = module.fourierft_indices[adapter_name]
                for adapter_name in module.fourierft_spectrum.keys():
                    # to circumvent super().state_dict() from removing adapter_name(.default)
                    # see peft/utils/save_and_load.py:278
                    state_dict_key = f"base_model.{name}.fourierft_spectrum.defa.{adapter_name}ult"
                    state_dict[state_dict_key] = module.fourierft_spectrum[adapter_name]
        return state_dict

    # def load_state_dict(self, state_dict, *args, **kwargs):
    #     """
    #     Overloads `load_state_dict()` methods to reflect the overloaded `state_dict()`
    #     """
    ...
    #             for adapter_name in module.fourierft_indices.keys():
    #                 state_dict_key = f"base_model.{name}.fourierft_indices"
    #                 print(f"load {state_dict_key}")
    #                 module.fourierft_indices[adapter_name] = state_dict.pop(state_dict_key)

    #             # name of fourierft_spectrum in state_dict seems to omit the adapter_name(e.g., "default")
    #             # fix loading issue by reimplementation
    #             for adapter_name in module.fourierft_spectrum.keys():
    #                 state_dict_key = f"base_model.{name}.fourierft_spectrum"
    #                 print(f"tried to pop {state_dict_key}: ", end="")
    #                 if state_dict_key in state_dict:
    #                     print("exists")
    #                 else:
    #                     print("does not exist")
    #                 module.fourierft_spectrum[adapter_name] = state_dict.pop(state_dict_key)
