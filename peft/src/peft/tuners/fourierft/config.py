...
class FourierFTConfig(PeftConfig):
    ...
    channel_wise: bool = field(
        ...
        metadata={
            "help": (
                "Set this to true if you want to apply 1D channelwise FFT instead of 2D FFT"
            )
        },
    )
    cosine_transform: bool = field(
        default=False,
        metadata={
            "help": (
                "Use discrete cosine transform instead of Fourier transform "
            )
        }
    )
    hadamard_transform: bool = field(
            default=False,
            metadata={
                "help": (
                    "Use Walsh-Hadamard transform instead of Fourier transform "
                )
            }
        )
