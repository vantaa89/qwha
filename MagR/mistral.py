...
@torch.no_grad()
def mistral_eval(model, testenc, dev):
    print('Evaluating ...')
...
def mistral_pack3(model, quantizers):
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'model', type=str,
        help='Mistral model to load; pass location of hugginface converted checkpoint.'
    )
    parser.add_argument(
    ...
    if args.wbits < 16 and not args.nearest:
        tick = time.time()
        quantizers = mistral_sequential(model, dataloader, DEV)
    ...
    for dataset in datasets:
        ...
        print(dataset)
        mistral_eval(model, testloader, DEV)
